...
{
    tolua_usertype(tolua_S,"${generator.get_script_namespace_name($current_class.namespaced_class_name)}");
    #if len($current_class.parents) > 0
    tolua_cclass(tolua_S,"${current_class.class_name}","${generator.get_script_namespace_name($current_class.namespaced_class_name)}","${generator.get_script_namespace_name($current_class.parents[0].namespaced_class_name)}",NULL);
    #else
    tolua_cclass(tolua_S,"${current_class.class_name}","${generator.get_script_namespace_name($current_class.namespaced_class_name)}","",NULL);
    #end if
